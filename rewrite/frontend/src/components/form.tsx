...
export default function MyForm() {
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		try {
			const LAMBDA_ENDPOINT = "https://6jwalrhrza.execute-api.eu-north-1.amazonaws.com";
	
			const formData = {
				full_name: values.name,
				email: values.email,
				committee: values.responsible,
				type: values.cardNumber ? "card" : "account",
				card_number: values.cardNumber || "",
				account: values.accountNumber || "",
				amount: values.amount,
				intent: values.anledning,
						...
						<FormItem>
							...
							<FormControl>
								<FileUploader
									...
									onValueChange={(files) => {
										setFiles(files);
										form.setValue("file", files?.map((file) => file.url) ?? []);
									}}
