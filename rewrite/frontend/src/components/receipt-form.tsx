...
export default function ReceiptForm() {
	...
	const [submittedFormState, setSubmittedFormState] = useState<unknown>(null);
	...
	async function sendEmail() {
		if (!pdfUrl) {
			toast.error("Ingen PDF-fil å sende");
			return;
		}
		...
		toast.info("Sender email til Bankkom...");
		...
		if (data.status === 200) {
			toast.success("Email sendt til Bankkom");
		} else {
			toast.error("Feil ved sending av email");
		}
	}
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		try {
			...
			setSubmittedFormState(formData);
